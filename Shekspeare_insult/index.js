...
function randomiseInsult() {
    if(insults.length > 0){
        const randomIndex = Math.floor(Math.random() * insults.length);
        const randomInsult = insults.splice(randomIndex, 1)[0];
        return `"${randomInsult.insult}" - ${randomInsult.play}`;
    } else {
        return 'We are out of insults, sorry'
    }
}
alert(randomiseInsult());